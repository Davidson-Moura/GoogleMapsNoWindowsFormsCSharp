    ...
    public partial class MainMapForm : Form
    {
        ...
        public MainMapForm()
        {
            ...
            if (!GMapControl.IsDesignerHosted)
            {
                ...
                // Initial Position
                //----------------------------------------
                string locationStart = "Brasil, RJ, São João de Meriti, Vilar dos Teles Rua Botafogo SN";// "Rua Botafogo - Vilar dos Teles, São João de Meriti - RJ, Brasil";
                var posStart = MainMap.GeocodingProvider.GetPoint(locationStart, out GeoCoderStatusCode status);//GoogleMapProvider.Instance.GetPoint(locationStart, out GeoCoderStatusCode status);

                /*
                 * return $"{businessPlace.Country}, {businessPlace.State}, {county.Name}, {businessPlace.Block},{businessPlace.AddrType} {businessPlace.Street} {businessPlace.StreetNo}";
                */
                if (posStart != null && status == GeoCoderStatusCode.OK)
                {
                    MainMap.Position = posStart.Value;// /*position is not null?*/ posStart.Value; // : new PointLatLng(54.6961334816182, 25.2985095977783); // Lithuania, Vilnius
                    textBoxGeo.Text = locationStart;
                }
                else
                {
                    textBoxGeo.Text = " Brasília, DF";
                }
                MainMap.MinZoom = 0;
                MainMap.MaxZoom = 24;
                MainMap.Zoom = 9;
                ...
                var pos = MainMap.GeocodingProvider.GetPoint("Rua botafogo, lote 10 quadra 229, São João de Meriti, RJ", out status);
                //PointLatLng? pos = GoogleMapProvider.Instance.GetPoint("Rua botafogo, lote 10 quadra 229, São João de Meriti, RJ", out GeoCoderStatusCode status2);
                ...
                if (pos != null && status == GeoCoderStatusCode.OK)
                {
                    ...
                    myCity.ToolTipText = "Ponto Inicial ;}";
        ...
        void transport_DoWork(object sender, DoWorkEventArgs e)
        {
            while (!_transportWorker.CancellationPending)
            {
                try
                {
                    ...
                    lock (_trolleybus)
                    {
                        Stuff.GetVilniusTransportData(Source.TransportType.TrolleyBus, string.Empty, _trolleybus);
                    }

                    lock (_bus)
                    {
                        Stuff.GetVilniusTransportData(Source.TransportType.Bus, string.Empty, _bus);
                    }
                    #endregion
